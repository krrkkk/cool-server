...
hook.Add("CreateMenuButtons", "ixCrafting", function(tabs)
	if (hook.Run("BuildCraftingMenu") != false) then
		tabs["Создание"] = function(container)
...
if (craftPanel.selected) then
	local text = craftPanel.search:GetText():lower()
	craftPanel:LoadRecipes(craftPanel.selected.category, text:find("%S") and text or nil)
end
